...
def insert_sensor_data(data, ip_address):
    ...
    try:
        ...
        conn.commit()
        logger.info(f"Successfully stored packet payload from station {mn} ({ip_address})")
    except Exception as e:
        if conn: conn.rollback()
...
def update_lead_value(mn, lead, temperature):
    ...
    try:
        ...
        conn.commit()
        logger.info(f"Lead status variables updated for station: {mn}")
    except Exception as e:
        if conn: conn.rollback()
        logger.exception(f"Asynchronous lead update thread write encountered error: {e}")
    finally:
        if conn: release_connection(conn)
...
def print_frame(data):
    ...
    cp = data["CP"]
    if "DataTime" in cp: print(f"DataTime : {cp['DataTime']}")
    print("\nSensor Component           Value\n" + "-" * 45)
...
def poll_station(mn, station):
    ...
    if not client.connect():
        print(f"[LEAD SENSOR] Unable to reach Modbus station gateway at: {ip}")
        return
    try:
        rr = read_registers(client, 0, 10, slave)
        if rr.isError():
            print(f"[LEAD SENSOR] Modbus Read register failure response on: {ip}")
            return
        regs = rr.registers
        lead = regs[2] / 10.0
        temperature = regs[1] / 10.0
        print(f"[LEAD SENSOR] {mn} Polled Data -> Lead={lead:.2f} ppm, Temp={temperature:.1f}")
        update_lead_value(mn=mn, lead=lead, temperature=temperature)
    except Exception as e:
        print(f"[LEAD SERVICE ERROR] Exception raised while executing poll loop {ip}: {e}")
    finally:
        client.close()

def lead_service():
    print("Background Modbus Polling Worker Started")
    while True:
...
def handle_client(conn, addr):
    ip_address = addr[0]
    print(f"\n[+] Client Socket Pipeline Opened: {ip_address}")
    buffer = ""
    while True:
        try:
            ...
            for frame in frames:
                ...
                if VERIFY_CHECKSUM and not verify_crc(frame):
                    print(f"[CRC FAILED] Package corrupted from source IP {ip_address}")
                    continue

                cn = get_field(frame, "CN")
                if cn == "2011":
                    try: process_frame(frame, ip_address)
                    except Exception as e: print(f"[PARSER EXCEPTION] Frame execution stalled: {e}")
                else:
                    print(f"[INFO] Command Number Type CN={cn} dropped (non-storage metric classification).")

                if cn in SUPPORTED_CN:
                    ack = build_ack(frame)
                    conn.sendall(ack.encode())
                    print(f"[TX ACKNOWLEDGE] {ack.strip()}")

        except socket.timeout: print(f"[CONN TIMEOUT] Station inactive: {ip_address}"); break
        except ConnectionResetError: print(f"[CONN RESET] Pipeline disrupted by peer: {ip_address}"); break
        except Exception as e: print(f"[SERVER RUNTIME ERROR] Client thread mapping issue on {ip_address}: {e}"); break

    conn.close()
    print(f"[-] Client Socket Pipeline Closed: {ip_address}")
...
def start_server():
    ...
    print("=" * 60 + "\nHJ212 MONOLITHIC TCP SERVER\n" + "=" * 60)
    print(f"Server is active and listening on: {SERVER_HOST}:{SERVER_PORT}\n" + "=" * 60)

    while True:
        conn, addr = server.accept()
        conn.settimeout(60)
        thread = threading.Thread(target=handle_client, args=(conn, addr), daemon=True, name=f"ClientWorker-{addr[0]}")
        thread.start()
...
if __name__ == "__main__":
    if initialize_database():
        ...
        print("Fatal Error: Database initialization criteria could not be met. Halting startup script.")